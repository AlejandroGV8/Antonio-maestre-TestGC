...
### Configuración recomendada: OIDC + IAM Role

La forma recomendada de desplegar desde GitHub a AWS es usando OIDC. De esta manera:

- no guardas access keys fijas en GitHub,
- no dependes de credenciales temporales manuales,
- no tienes que desactivar MFA de tu usuario personal.

### 1) Crear el proveedor OIDC en AWS

En IAM > Identity providers, crea un proveedor con estos valores:

- Provider URL: `https://token.actions.githubusercontent.com`
- Audience: `sts.amazonaws.com`

### 2) Crear un IAM Role para GitHub Actions

Crea un rol con una trust policy similar a esta, sustituyendo `AlejandroGV8/Antonio-maestre-TestGC` por tu repo exacto si cambia:
	...
	"Statement": [
		{
			...
			"Condition": {
				...
				"StringLike": {
					"token.actions.githubusercontent.com:sub": "repo:AlejandroGV8/Antonio-maestre-TestGC:ref:refs/heads/main"
				}
			}
		}
	]
...
### 3) Añadir permisos al rol

Policy mínima para desplegar a S3 y opcionalmente invalidar CloudFront:
...
Si no usas CloudFront, puedes eliminar ese último bloque.

### Secrets necesarios en GitHub

Configura estos secrets en GitHub Repository Settings > Secrets and variables > Actions:

- `AWS_ROLE_TO_ASSUME` (ARN completo del rol, por ejemplo `arn:aws:iam::123456789012:role/github-actions-s3-deploy`)
